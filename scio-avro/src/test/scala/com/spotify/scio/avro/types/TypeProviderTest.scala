...
class TypeProviderTest extends FlatSpec with Matchers {
  ...
  it should "support required primitive types" in {
    val r = RecordWithBasicTypes(true, 1, 2L, 1.5f, 2.5, "string", ByteString.copyFromUtf8("bytes"))
  ...
  it should "support .tupled in companion object" in {
    val r1 = RecordWithBasicTypes(true, 1, 2L, 1.5f, 2.5, "string", ByteString.copyFromUtf8("bytes"))
    val r2 = RecordWithBasicTypes.tupled((true, 1, 2L, 1.5f, 2.5, "string", ByteString.copyFromUtf8("bytes")))
    r1 shouldBe r2
  }
  ...
  it should "support nested records" in {
    val r = RecordWithRecords(Basic$1(1), Optional$1(Some(1)), Array$1(List(1)),  Map$1(Map("int" -> 1)))
